...
test('getProductLinksToBeParsed', async () => {
    ...
    const result = await getProductLinksToBeParsed(
        [
            {
                path: path.resolve(__dirname, '../input-files/analytic-prices/1_ОСТАТКИ_ЦС_09_12_22.xlsx'),
            },
            {
                path: path.resolve(__dirname, '../input-files/analytic-prices/2_Таблица_ссылок_для_парсинга_цен.xlsx')
            }
        ]
    );
